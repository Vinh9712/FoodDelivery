-- Sequenced inbox for order.events.v1 consumption (delivery-order-v1).
...
CREATE TABLE deferred_integration_events (
    ...
    CONSTRAINT uq_delivery_deferred_consumer_event
        UNIQUE (consumer_name, event_id),
    CONSTRAINT uq_delivery_deferred_consumer_aggregate_sequence
        UNIQUE (consumer_name, aggregate_type, aggregate_id, aggregate_sequence),
    CONSTRAINT chk_delivery_deferred_status
        CHECK (status IN ('WAITING_FOR_PREDECESSOR', 'DEAD_LETTER'))
);

CREATE INDEX idx_delivery_deferred_due
